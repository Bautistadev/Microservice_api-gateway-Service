pipeline{
    ...
    stages{
        stage('checkout'){
            steps{
                git branch: 'main', credentialsId: 'bautistadevgithub', url: 'https://github.com/Bautistadev/Microservice_api-gateway-Service'
            }
        }
        stage('build'){
            steps{
        ...
        stage('docker build'){
           steps{
               script{
                   ...
                   docker.image("bautistadev/api_gateway_microservice:v${BUILD_NUMBER}")
